...
public class SingleStepPathingStrategy implements PathingStrategy 
{
	...
	{
		/*
		 * Does not check withinReach. Since only a single step is taken on each call,
		 * the caller will need to check if the destination has been reached.
		 */
		return potentialNeighbors.apply(start).filter(canPassThrough)
